...
from app_version import RELEASE_ASSET_PREFIX, REPOSITORY
...
def _request_bytes(url: str, timeout: float = 15.0) -> bytes:
...
class UpdateService:
    ...
    def check(self, current_version: str) -> UpdateInfo | None:
        try:
            payload = json.loads(
                _request_bytes(self.latest_release_api).decode("utf-8")
            )
